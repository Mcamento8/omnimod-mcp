...
npm run e2e        # end-to-end JSON-RPC test (68 tools, 13 resources, 5 prompts)
```
...
Once the server is connected, the agent has 68 tools (plus the full map-dev master guide: `omni_map_guide` / resource `omnimod://knowledge/map-dev-guide`). A typical first
session:
